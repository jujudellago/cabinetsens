...
if (!function_exists('trx_addons_options_page_load_scripts')) {
	...
	function trx_addons_options_page_load_scripts() { 
		if (apply_filters('trx_addons_filter_need_options', isset($_REQUEST['page']) && $_REQUEST['page']=='trx_addons_options') || current_user_can('edit_users')) { 
			// WP styles & scripts
		...
		if (count($tabs_content) > 0) {
			...
			<div id="trx_addons_options_tabs" class="<?php echo count($tabs_titles) > 1 ? 'with_tabs' : 'no_tabs'; ?>">
				<?php if (count($tabs_titles) > 1) { ?>
					<ul><?php
						foreach ($tabs_titles as $k=>$v) {
							?><li<?php if ($tabs_empty[$k]) echo ' class="trx_addons_options_item_hidden"'; ?>><a href="#trx_addons_options_section_<?php echo esc_attr($k); ?>"><?php echo esc_html($v); ?></a></li><?php
						}
					?></ul>
				<?php
				}
				foreach ($tabs_content as $k=>$v) {
					?>
					<div id="trx_addons_options_section_<?php echo esc_attr($k); ?>" class="trx_addons_options_section">
