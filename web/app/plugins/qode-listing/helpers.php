...
if(!function_exists('qode_listing_is_elementor_installed')){
	/**
	 * check if is installed Elementor Plugin
	 */
	function qode_listing_is_elementor_installed(){
...
if( ! function_exists('qode_listing_add_elementor_widget_categories') ) {
    function qode_listing_add_elementor_widget_categories($elements_manager) {

        $elements_manager->add_category(
            'qode-listing',
            [
                'title' => esc_html__('Qode Listing', 'qode-listing'),
                'icon' => 'fa fa-plug',
            ]
        );
    ...
    add_action('elementor/elements/categories_registered', 'qode_listing_add_elementor_widget_categories');
};

if(!function_exists('qode_listing_include_elementor_course_shortcodes')) {
    function qode_listing_include_elementor_course_shortcodes() {
        if( qode_listing_is_wp_job_manager_installed() ) {
            foreach (glob(QODE_LISTING_SHORTCODES_PATH . '/*/elementor-*.php') as $shortcode_load) {
                if (strpos($shortcode_load, 'listing-package') == false) {
    ...
    add_action('bridge_core_load_elementor_shortcodes_from_plugins', 'qode_listing_include_elementor_course_shortcodes');
}