...
//function that removes WP Job Manager the_content filter so Elementor the_content filter will be applied and their content will be available
if( ! function_exists('qode_listing_remove_wp_job_manager_content_filter') ){
    function qode_listing_remove_wp_job_manager_content_filter(){
        remove_filter( 'the_content', array( WP_Job_Manager_Post_Types::instance(), 'job_content' ) );
    }

    add_action( 'wp_head', 'qode_listing_remove_wp_job_manager_content_filter' );
}