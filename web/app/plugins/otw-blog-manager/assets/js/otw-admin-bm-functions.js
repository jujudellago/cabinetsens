...
jQuery(document).ready(function(){
	...
	otw_select2( '.js-categories', { url: req_url, action: 'otw_bm_select2_options', otw_option_type: 'category' } );
	otw_select2( '.js-exclude_categories', { url: req_url, action: 'otw_bm_select2_options', otw_option_type: 'category' } );
	otw_select2( '.js-tags', { url: req_url, action: 'otw_bm_select2_options', otw_option_type: 'tag' } );
	otw_select2( '.js-exclude_tags', { url: req_url, action: 'otw_bm_select2_options', otw_option_type: 'tag' } );
	otw_select2( '.js-users', { url: req_url, action: 'otw_bm_select2_options', otw_option_type: 'user' } );
	otw_select2( '.js-exclude_users', { url: req_url, action: 'otw_bm_select2_options', otw_option_type: 'user' } );
	
	otw_select2( '.js-pages', { url: req_url, action: 'otw_bm_select2_options', otw_option_type: 'page' } );
	
	otw_select2( '.js-fonts', { data: JSON.parse( fonts ), action: 'otw_bm_select2_options', otw_option_type: 'page' } );
...
function setupBlogElements () {
	...
	if( typeof blogElements !== 'undefined' && typeof metaElements !== 'undefined' ) {
		...
		var metaItems = metaElements.split(',');
...
function otw_select2( otw_selector, otw_params ){
	...
	otw_select_2_object.each( function(){
	
		var s2_object = jQuery( this );
		
		var initial_value = s2_object.attr( 'data-value' );
				...
				jQuery.ajax( otw_params.url , {
					...
					if( typeof( data.results ) == 'object' ){
						...
						for( var cD = 0; cD < data.results.length; cD++ ){
							...
							s2_object.append( '<option value="' + data.results[ cD ].id + '" selected="selected">' + data.results[ cD ].text + '</option>');
						};
					};
				});
				
			}else{
				s2_object.val( initial_value );
				s2_object.trigger( 'change' );
			}
		};
	} );
};
