<?php if ( $next_question !== - 1 ) {
	...
	?>
	<form action='' method='post' class="qode-lms-question-next-form">
		...
		<input type='hidden' name='qode_lms_change_question' value='<?php echo esc_attr( $next_question ); ?>'/>
		...
		<div class="qode-question-actions">
			...
			echo bridge_core_get_button_html(
				array(
					'custom_class' => 'qode-next-question qode-lms-actions-buttons',
					'html_type'    => 'button',
					'input_name'   => 'submit',
					'size'         => 'medium',
					'text'         => esc_html__( 'Next >', 'qode-lms' )
				)
			);
			?>
		</div>
