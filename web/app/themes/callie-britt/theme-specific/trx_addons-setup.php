...
------------------------------------------------------------------------------- */
if (!function_exists('callie_britt_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'callie_britt_trx_addons_theme_specific_components');
	function callie_britt_trx_addons_theme_specific_components($enable=false) {
		return CALLIE_BRITT_THEME_FREE
					? false		// Free version
...
if (!function_exists('callie_britt_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'callie_britt_trx_addons_theme_specific_setup1', 1 );
	function callie_britt_trx_addons_theme_specific_setup1() {
		if (callie_britt_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'callie_britt_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'callie_britt_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'callie_britt_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'callie_britt_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'callie_britt_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'callie_britt_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'callie_britt_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'callie_britt_trx_addons_cv_enable');
	function callie_britt_trx_addons_cv_enable($enable=false) {
...
// Demo mode
if ( !function_exists( 'callie_britt_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'callie_britt_trx_addons_demo_enable');
	function callie_britt_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'callie_britt_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'callie_britt_trx_addons_edd_themes_market_enable')) {
	function callie_britt_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'callie_britt_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'callie_britt_trx_addons_api_list');
	function callie_britt_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'callie_britt_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'callie_britt_trx_addons_cpt_list');
	function callie_britt_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'callie_britt_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'callie_britt_trx_addons_sc_list');
	function callie_britt_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'callie_britt_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'callie_britt_trx_addons_widgets_list');
	function callie_britt_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'callie_britt_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'callie_britt_trx_addons_menu_cache');
	function callie_britt_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('callie_britt_trx_addons_localize_script')) {
	...
	function callie_britt_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = callie_britt_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'callie_britt_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'callie_britt_trx_addons_sc_type', 10, 2);
	function callie_britt_trx_addons_sc_type($list, $sc) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ($sc == 'trx_sc_blogger') {
			$list = callie_britt_array_merge($list, callie_britt_get_list_blog_styles(false, 'sc'));
		}
		return $list;
	}
}
...
if ( !function_exists( 'callie_britt_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'callie_britt_trx_addons_sc_prepare_atts', 10, 2);
	function callie_britt_trx_addons_sc_prepare_atts($atts, $sc) {
		if ( $sc == 'trx_sc_blogger') {
			$list = callie_britt_get_list_blog_styles(false, 'sc');
...
if ( !function_exists( 'callie_britt_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'callie_britt_trx_addons_sc_atts', 10, 2);
	function callie_britt_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'callie_britt_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'callie_britt_trx_addons_sc_map', 10, 2);
	function callie_britt_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'callie-britt'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'callie-britt') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(callie_britt_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
		...
		if ($sc == 'trx_sc_blogger') {
			...
			$param = array(
					'param_name' => 'hover',
					'heading' => esc_html__('Image hover', 'callie-britt'),
					'description' => wp_kses_data( __('Select hover effect for the featured image', 'callie-britt') ),
					'edit_field_class' => 'vc_col-sm-4',
					'value' => array_flip(callie_britt_get_list_hovers(true)),
					'dependency' => array(
						'element' => 'type',
...
if ( !function_exists( 'callie_britt_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'callie_britt_trx_addons_sow_map', 10, 2);
	function callie_britt_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'callie-britt'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'callie-britt') ),
				'options' => callie_britt_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			callie_britt_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			callie_britt_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output from new params
if ( !function_exists( 'callie_britt_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'callie_britt_trx_addons_sc_output', 10, 4);
	function callie_britt_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !callie_britt_is_inherit($atts['color_style']) && $atts['color_style']!='default')
...
if ( !function_exists( 'callie_britt_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'callie_britt_trx_addons_sc_item_title_tag');
	function callie_britt_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'callie_britt_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'callie_britt_trx_addons_sc_item_button_args', 10, 3);
	function callie_britt_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'callie_britt_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'callie_britt_trx_addons_slider_title', 10, 2 );
	function callie_britt_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'callie_britt_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'callie_britt_trx_addons_sc_googlemap_styles');
	function callie_britt_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'callie-britt');
		return $list;
	}
}
...
if ( !function_exists( 'callie_britt_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'callie_britt_trx_addons_tiny_mce_style_formats');
	function callie_britt_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (false && is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
				if ( $v['title'] == esc_html__('List styles', 'callie-britt') ) {
					$style_formats[$k]['items'][] = array(
								'title' => esc_html__('Arrow', 'callie-britt'),
								'selector' => 'ul',
...
if ( !function_exists( 'callie_britt_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'callie_britt_filter_allow_override_header_image', 'callie_britt_trx_addons_allow_override_header_image' );
	function callie_britt_trx_addons_allow_override_header_image($allow) {
		return callie_britt_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'callie_britt_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'callie_britt_trx_addons_thumb_size', 10, 2);
	function callie_britt_trx_addons_thumb_size($thumb_size='', $type='') {
//		Uncomment next lines to change team members images (default is 'avatar')
//		if ($type == 'team-default')
//			$thumb_size = callie_britt_get_thumb_size('big');
		return $thumb_size;
	}
}
...
if (!function_exists('callie_britt_trx_addons_meta_box_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'callie_britt_trx_addons_meta_box_fields', 10, 2);
	function callie_britt_trx_addons_meta_box_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			if (!isset($mb['email'])) {
				$mb['email'] = array(
					"title" => esc_html__("E-mail",  'callie-britt'),
					"desc" => wp_kses_data( __("Team member's email", 'callie-britt') ),
					"std" => "",
					"details" => true,
					"type" => "text"
				);
	...
	function trx_addons_get_privacy_text() {
		$page = get_option('wp_page_for_privacy_policy');
			return apply_filters( 'trx_addons_filter_privacy_text', wp_kses_post( 
				esc_html__( 'I agree that my submitted data is being collected and stored.', 'callie-britt' )
				. ( '' != $page
					// Translators: Add url to the Privacy Policy page
					? ' ' . sprintf(__('For further details on handling user data, see our %s', 'callie-britt'),
					'<a href="' . esc_url(get_permalink($page)) . '" target="_blank">'
					. esc_html__('Privacy Policy', 'callie-britt')
					. '</a>') 
					: ''
					)
				)
...
if ( ! function_exists( 'callie_britt_trx_addons_sc_param_group_params' ) ) {	
	add_filter('trx_addons_sc_param_group_params', 'callie_britt_trx_addons_sc_param_group_params', 10, 2);
	function callie_britt_trx_addons_sc_param_group_params($arr = array(), $type = ''){
		if( 'trx_sc_action' === $type ) {
			unset($arr[8]);
		}
		return $arr;
	}
