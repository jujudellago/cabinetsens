 ...
 * @since CALLIE-BRITT 1.0.39
 */

// Additional info
if (callie_britt_is_off(callie_britt_get_theme_option('header_mobile_hide_info')) && ($callie_britt_info=callie_britt_get_theme_option('header_mobile_additional_info'))!='') {
			...
			<div class="columns_wrap">
				<div class="sc_layouts_column sc_layouts_column_align_center sc_layouts_column_icons_position_left column-1_1"><?php
					?><div class="sc_layouts_item"><?php
						callie_britt_show_layout($callie_britt_info);
					?></div><!-- /.sc_layouts_item -->
	...
	<div class="content_wrap">
		<div class="columns_wrap columns_fluid">
			<div class="sc_layouts_column sc_layouts_column_align_left sc_layouts_column_icons_position_left sc_layouts_column_fluid column-1_3"><?php
				// Logo
				if (callie_britt_is_off(callie_britt_get_theme_option('header_mobile_hide_logo'))) {
					?><div class="sc_layouts_item"><?php
						set_query_var('callie_britt_logo_args', array('type' => 'mobile_header'));
			...
			?><div class="sc_layouts_column sc_layouts_column_align_right sc_layouts_column_icons_position_left sc_layouts_column_fluid  column-2_3"><?php
				if (callie_britt_exists_trx_addons()) {
					...
					if (callie_britt_is_off(callie_britt_get_theme_option('header_mobile_hide_login'))) {
						ob_start();
						do_action('callie_britt_action_login', array('text_login' => false, 'text_logout' => false));
						$callie_britt_action_output = ob_get_contents();
						ob_end_clean();
						if (!empty($callie_britt_action_output)) {
							?><div class="sc_layouts_item sc_layouts_menu sc_layouts_menu_default"><?php
								callie_britt_show_layout($callie_britt_action_output);
							?></div><?php
						}
					}
					// Display cart button
					if (callie_britt_is_off(callie_britt_get_theme_option('header_mobile_hide_cart'))) {
						ob_start();
						do_action('callie_britt_action_cart');
						$callie_britt_action_output = ob_get_contents();
						ob_end_clean();
						if (!empty($callie_britt_action_output)) {
							?><div class="sc_layouts_item"><?php
					...
					if (callie_britt_is_off(callie_britt_get_theme_option('header_mobile_hide_search'))) {
						ob_start();
						do_action('callie_britt_action_search', 'fullscreen', 'header_mobile_search', false);
						$callie_britt_action_output = ob_get_contents();
						ob_end_clean();
						if (!empty($callie_britt_action_output)) {
							?><div class="sc_layouts_item"><?php
