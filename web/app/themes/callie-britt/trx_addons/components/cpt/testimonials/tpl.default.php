...
if ($query->found_posts > 0) {
	...
	?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?>
		...
		if ($args['slider']) {
			$pagination_bullets = $pagination_thumbs = '';
		...
		while ( $query->have_posts() ) { $query->the_post();
			if ($args['slider']) {
				...
				$pagination_thumbs 	.= '<span class="slider-pagination-button swiper-pagination-button"'
									. ($img ? ' style="background-image: url('.esc_url($img).');"' : '')
									. '>'
									. '</span>';

				$pagination_bullets .= '<span class="slider-pagination-button swiper-pagination-button"></span>';
			}
		...
		if ($args['slider']) {
			if (empty($args['slider_pagination_thumbs']))
				...
				$args['slider_pagination_buttons'] = '';
			$args['slider_pagination_buttons_thumbs'] = $pagination_thumbs;
