		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				if (!empty($item['image'])) {
					...
					?><div class="sc_action_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e("Action's image", 'callie-britt'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
				...
				if (!empty($icon)) {
					...
					if (!empty($item['icon_link'])) { 
						echo '<a href="'.esc_url($item['icon_link']).'" target="blanc" class="sc_action_item_icon sc_action_item_icon_type_'.esc_attr($item['icon_type']).' '.esc_attr($icon).'">';	
					}
					else { 
						echo '<div class="sc_action_item_icon sc_action_item_icon_type_'.esc_attr($item['icon_type']).' '.esc_attr($icon).'">';	
					}
						if (!empty($svg)) {
							...
							?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'callie-britt'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						} else {
