...
?><div <?php post_class( 'sc_blogger_item post_format_'.esc_attr($post_format) . (empty($post_link) ? ' no_links' : '') ); ?>><?php
	...
	if (callie_britt_exists_tribe_events() && $args['post_type'] != Tribe__Events__Main::POSTTYPE) {
	...
	?><div class="sc_blogger_item_content entry-content"><?php
		...
		if ( !in_array($post_format, array('link', 'aside', 'status', 'quote')) ) {
			?><div class="sc_blogger_item_header entry-header"><?php 
				...
				if (callie_britt_exists_tribe_events() && $args['post_type'] == Tribe__Events__Main::POSTTYPE && in_array('comments',explode(',', $callie_britt_counters))) {
					$callie_britt_counters = str_replace('comments', '', $callie_britt_counters);
				}
